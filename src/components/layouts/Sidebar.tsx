    ...
    <div className="w-16 md:w-64 h-screen bg-white/5 backdrop-blur-md border-r border-white/10 flex flex-col">
      ...
      <nav className="flex-1 py-6">
        <ul className="space-y-2">
          <SidebarItem to="/" icon={<Home className="w-5 h-5" />} label="Dashboard" />
          <SidebarItem to="/stocks" icon={<BarChart3 className="w-5 h-5" />} label="Stocks" />
          <SidebarItem to="/analysis" icon={<LineChart className="w-5 h-5" />} label="Analysis" />
          <SidebarItem to="/ai-assistant" icon={<MessageSquare className="w-5 h-5" />} label="AI Assistant" />
          <SidebarItem to="/search" icon={<Search className="w-5 h-5" />} label="Search" />
        </ul>
      </nav>
...
const SidebarItem: React.FC<SidebarItemProps> = ({ to, icon, label }) => {
  return (
    <li>
      <Link
        to={to}
        className="flex items-center px-4 py-3 text-white/80 hover:text-white hover:bg-white/10 rounded-md transition-all"
      >
