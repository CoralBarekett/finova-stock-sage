...
import { DollarSign, TrendingUp, TrendingDown, BarChart3, Brain, Activity } from 'lucide-react';
...
const StatsPanel: React.FC<StatsPanelProps> = ({ isDark, stats }) => {
  ...
  const statCards = [
    {
      ...
      icon: DollarSign,
      bgColor: isDark ? 'from-blue-900/30 to-blue-800/20' : 'from-blue-50 to-blue-100',
      iconBg: 'bg-blue-500',
    ...
    },
    {
      ...
      icon: Brain,
      bgColor: isDark ? 'from-purple-900/30 to-purple-800/20' : 'from-purple-50 to-purple-100',
      iconBg: 'bg-purple-500',
      textColor: 'text-purple-600'
    },
    {
      title: 'Predicted Change',
      value: formatPercentage(stats.changePercent),
      icon: isPositiveChange ? TrendingUp : TrendingDown,
      bgColor: isPositiveChange 
        ? isDark ? 'from-emerald-900/30 to-emerald-800/20' : 'from-emerald-50 to-emerald-100'
        : isDark ? 'from-red-900/30 to-red-800/20' : 'from-red-50 to-red-100',
      iconBg: isPositiveChange ? 'bg-emerald-500' : 'bg-red-500',
      textColor: isPositiveChange ? 'text-emerald-600' : 'text-red-600'
    },
    {
      title: 'AI Confidence',
      value: `${Math.round(confidenceLevel)}%`,
      icon: Activity,
      bgColor: isDark ? 'from-orange-900/30 to-orange-800/20' : 'from-orange-50 to-orange-100',
              ...
              <div className={`w-full h-2 rounded-full ${
                ...
                <div 
                  className={`h-full rounded-full transition-all duration-500 ${
                    confidenceLevel >= 80 ? 'bg-emerald-500' :
                    confidenceLevel >= 60 ? 'bg-yellow-500' : 'bg-orange-500'
                  }`}
