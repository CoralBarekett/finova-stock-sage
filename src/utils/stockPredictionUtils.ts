...
export const prepareChartData = (
  ...
  historicalData.forEach((item, index) => {
    ...
    if (index % 10 === 0) {
      console.log(`[DEBUG] Added historical point ${index + 1}/${historicalData.length}: ${item.date} - $${item.price}`);
    }
  });
  ...
  if (predictionData && historicalData.length > 0) {
    ...
    if (lastHistoricalDate) {
      ...
      console.log(`[DEBUG] Added prediction point: ${predictionPoint.date} - $${predictionPoint.predictedPrice}`);
    }
  } else {
    console.log('[DEBUG] No prediction data or historical data available for chart');
  }
...
export const calculateStats = (
  ...
  if (predictionData) {
    ...
    if (predictionData.predicted_price) {
      ...
      predictedPrice = calculatePredictedPrice(currentPrice, predictionData);
    }
    ...
    confidence = predictionData.prediction?.confidence || predictionData.confidence || 0;
    direction = predictionData.prediction?.direction || predictionData.technical_signals?.trend || 'hold';
    sentiment = predictionData.prediction?.sentiment || 
                (typeof predictionData.sentiment_analysis === 'string' ? predictionData.sentiment_analysis : 
                 predictionData.analysis?.social_sentiment?.sentiment) || 'neutral';
    
    // Posts analyzed from various sources
    postsAnalyzed = predictionData.analysis?.posts_analyzed || 
                   predictionData.data_sources?.total_posts ||
                   predictionData.supporting_data?.post_count || 0;
    
    influencerPosts = predictionData.data_sources?.influencer_posts ||
                     predictionData.supporting_data?.influencer_post_count || 0;
    
    technicalTrend = predictionData.technical_signals?.trend || 
                    predictionData.prediction?.direction || 'neutral';
    
    priceChangePercent = predictionData.technical_signals?.price_change_percent || changePercent;
    
    console.log(`[DEBUG] Stats calculated:`);
    console.log(`[DEBUG] - Predicted price: $${predictedPrice}`);
    console.log(`[DEBUG] - Change: $${change.toFixed(2)} (${changePercent.toFixed(2)}%)`);
...
export const calculateDateRange = (currentDate: Date, timeframe: TimeframeType) => {
  ...
  console.log(`[DEBUG] Date range calculated: ${startDate.toISOString().split('T')[0]} to ${endDate.toISOString().split('T')[0]} (${days} days)`);
...
export const validatePredictionData = (predictionData: PredictionResponse | null): boolean => {
  ...
  const hasConfidence = (predictionData.prediction?.confidence || predictionData.confidence || 0) > 0;
  const hasMinimumConfidence = (predictionData.prediction?.confidence || predictionData.confidence || 0) > 0.3;
  const hasPrediction = !!(predictionData.predicted_price || 
                          predictionData.prediction?.price_target || 
                          predictionData.prediction);
  const hasAnalysis = !!(predictionData.analysis?.posts_analyzed || 
                        predictionData.data_sources?.total_posts ||
                        predictionData.supporting_data?.post_count);
  
  const isValid = hasConfidence && hasMinimumConfidence && hasPrediction && hasAnalysis;
